...
def logisticRegression():
    ...
    global X, Y, X_train, X_test, y_train, y_test
    text.delete('1.0', END)
    cls = LogisticRegression(C=1, penalty='l1', solver='liblinear')
    cls.fit(X_train, y_train)
    text.insert(END,"Prediction Results\n\n") 
    prediction_data = prediction(X_test, cls) 
...
def EPP():
    ...
    epp = BaggingClassifier(estimator=base)  # Use 'estimator' instead of 'base_estimator'
...
def graph():
    height = [svm_mae,random_mae,logistic_mae]
    bars = ('SVM MAE', 'Random Forest MAE','Logistic MAE')
