...
def logisticRegression():
    global classifier,cls1
    global logistic_mae
    global X, Y, X_train, X_test, y_train, y_test
    cls1 = LogisticRegression(penalty='l2', tol=0.02, C=3.0)
...
def EPP():
    ...
    epp = BaggingClassifier(estimator=base)  # Corrected here
...
def predictPerformance():
    ...
    value = cls1.predict(records)
    text.insert(END, str(value)+"\n")
    for i in range(len(test)):
        result = value[i]
        if result <= 30:
            text.insert(END, "Predicted New Course GPA Score will be: High & Suggested/Recommended Future Course is: "+courses[result]+"\n")
        if result > 30:
            text.insert(END, "Predicted New Course GPA Score will be: Low & Suggested/Recommended Future Course is: "+courses[result]+"\n")    
...
main.configure(bg='#802752')  # Set background color
font = ('times', 20, 'bold')
title = Label(main, text='Student Career Prediction By Using Machine Learning', font=("times", 20, 'bold'))
title.config(bg='dark blue', fg='white') #set the title color
...
uploadButton = Button(main, text="Upload Dataset", command=upload, bg="purple")
...
predictButton.place(x=880, y=500)
